...
public class InputActivity extends AppCompatActivity {
    ...
    private View.OnClickListener mOnTimeClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            timePickerDialog.show(); //ここでshow書くの？？
        }
    };
    ...
    private void addTask() {
        ...
        resultIntent.putExtra(MainActivity.EXTRA_TASK, mTask.getId());//Extraにタスクを設定。これはTaskAlarmReceiverがブロードキャストを受け取った後、タスクのタイトルなどを表示する通知を発行するためにタスクの情報が必要になるから。
        PendingIntent resultPendingIntent = PendingIntent.getBroadcast(
                this,
                mTask.getId(),
                resultIntent,
                PendingIntent.FLAG_UPDATE_CURRENT //PendingIntent.FLAG_UPDATE_CURRENTは既存のPendingIntentがあれば、それはそのままでextraのデータだけ置き換えるという指定
        );
